...
export const RECIPES: RecipeEntry[] = [
  ...
  {
    "id": "glitch-bad-tv",
    "description": "Add a control-driven analog bad-TV glitch to every selected layer.",
    "keywords": [
      "bad tv",
      "vhs glitch",
      "analog glitch",
    ...
    "verifiedStatus": "verified",
    "notes": "\"Signal Noise\", with a default of 15, is the dial-in control for HLS noise lightness. Wave Warp gives the rolling analog warp, and Venetian Blinds cuts thin scanline gaps; tune Transition Completion and Width, or delete that effect, if the scanlines read too strong or subtle.."
  },
  {
    "id": "glitch-block-tear",
    "description": "Add a control-driven block-tear glitch to every selected layer.",
    "keywords": [
      "block tear",
      "datamosh",
      "digital tear",
    ...
    "verifiedStatus": "verified",
    "notes": "\"Tear Amount\" is the one dial-in control, with a default of 40 and a practical range of about 0-100, and it drives Turbulent Displace Amount through an expression. Frame Rate 12 and Size 8 are fixed defaults tuned for a blocky tear look; edit them directly on the effect for a different fps or block size.."
  },
  {
    "id": "glitch-random-position",
    "description": "Add intermittent control-driven position jumps to every selected layer.",
    "keywords": [
    ...
    "verifiedStatus": "verified",
    "notes": "\"Jitter\", with a default of 24px, controls the maximum random offset per jump. The expression gates jumps to happen only about 35% of the time with random() > 0.65 through posterizeTime(12), so it reads as intermittent snaps rather than continuous shake, not a smooth wiggle.."
  },
  {
    "id": "glitch-rgb-split",
    "description": "Create an adjustable RGB channel-split glitch on every selected layer.",
    "keywords": [
      "rgb split",
    ...
    "script": "app.beginUndoGroup(\"Glitch RGB Split\");\ntry {\n  var comp = app.project.activeItem;\n  if (!(comp instanceof CompItem)) throw new Error(\"Open a composition first.\");\n  if (comp.selectedLayers.length < 1) throw new Error(\"Select at least one layer.\");\n  var selectedLayers = comp.selectedLayers;\n  var selectedCount = selectedLayers.length;\n  for (var i = 0; i < selectedCount; i++) {\n    var original = selectedLayers[i];\n    var baseName = original.name;\n    var green = original.duplicate();\n    var blue = original.duplicate();\n    original.name = baseName + \" [R]\";\n    green.name = baseName + \" [G]\";\n    blue.name = baseName + \" [B]\";\n    blue.moveAfter(original);\n    var redEffects = original.property(\"ADBE Effect Parade\");\n    var redChannels = redEffects.addProperty(\"ADBE Shift Channels\");\n    redChannels.property(\"ADBE Shift Channels-0001\").setValue(1);\n    redChannels.property(\"ADBE Shift Channels-0002\").setValue(2);\n    redChannels.property(\"ADBE Shift Channels-0003\").setValue(10);\n    redChannels.property(\"ADBE Shift Channels-0004\").setValue(10);\n    var greenEffects = green.property(\"ADBE Effect Parade\");\n    var greenChannels = greenEffects.addProperty(\"ADBE Shift Channels\");\n    greenChannels.property(\"ADBE Shift Channels-0001\").setValue(1);\n    greenChannels.property(\"ADBE Shift Channels-0002\").setValue(10);\n    greenChannels.property(\"ADBE Shift Channels-0003\").setValue(3);\n    greenChannels.property(\"ADBE Shift Channels-0004\").setValue(10);\n    var blueEffects = blue.property(\"ADBE Effect Parade\");\n    var blueChannels = blueEffects.addProperty(\"ADBE Shift Channels\");\n    blueChannels.property(\"ADBE Shift Channels-0001\").setValue(1);\n    blueChannels.property(\"ADBE Shift Channels-0002\").setValue(10);\n    blueChannels.property(\"ADBE Shift Channels-0003\").setValue(10);\n    blueChannels.property(\"ADBE Shift Channels-0004\").setValue(4);\n    blue.blendingMode = BlendingMode.NORMAL;\n    original.blendingMode = BlendingMode.ADD;\n    green.blendingMode = BlendingMode.ADD;\n    var redTransform = redEffects.addProperty(\"ADBE Geometry2\");\n    redTransform.property(\"ADBE Geometry2-0002\").setValue([comp.width / 2 + 8, comp.height / 2]);\n    var greenTransform = greenEffects.addProperty(\"ADBE Geometry2\");\n    greenTransform.property(\"ADBE Geometry2-0002\").setValue([comp.width / 2 - 8, comp.height / 2]);\n  }\n} finally {\n  app.endUndoGroup();\n}",
    "terms": [
      "create",
      "adjustable",
      "rgb",
    ...
    "verifiedStatus": "verified",
    "notes": "Creates 3 layers per selected layer as R, G, and B copies recombined with ADD blend modes. The Transform effect Position on the [R] and [G] copies is the adjustable offset dial; drag it in the Effect Controls panel to change the split distance. Apply this recipe LAST if stacking with other glitch recipes, since it works on layer duplicates, not the original layer's own effects stack.."
  },
  {
    "id": "glitch-time-stutter",
    "description": "Add a control-driven time-stutter and echo-trail glitch to every selected layer.",
    "keywords": [
      "time stutter",
      "strobe glitch",
      "frame stutter",
    ...
    "verifiedStatus": "verified",
    "notes": "\"Stutter FPS\", with a default of 8, is the dial-in control for how choppy the frame rate reads. Echo adds 4 ghost-trail repeats decaying at 0.6 and screen-blended. Lower Stutter FPS is choppier.."
  },
  {
  ...
  },
  {
    ...
    "verifiedStatus": "verified",
    "notes": "Progress is the master 0-100 driver; Max Block Size sets the full-pixelation block size in pixels; Sharp Colors toggles Mosaic hard color boundaries; Fade Out enables the final opacity fade. The 0/50/70/100 stagger starts pixelation at 0, Block Dissolve at 50, and optional opacity fading at 70, with all stages complete at 100. Progress is auto-keyframed by default (0 at comp-duration-minus-1-second, 100 at the very end of the comp) so the transition is immediately visible on playback -- drag or retime those two keyframes like any other keyframe to change when the transition happens. IMPORTANT: the Mosaic and Block Dissolve effects live on the new content layer in the comp you ran this on, NOT inside the precomp it creates -- opening the precomp itself will always look static since effects apply to the layer instance, not the nested composition's own contents."
  },
  {
    "id": "copy-keyframes",
  ...
  {
    "id": "variable-font-axis-animation",
    "description": "Animate a variable font's weight/width axes on a text layer via a new Text Animator",
    "keywords": [
      "variable font",
      "font axis",
      "font axes",
    ...
    "verifiedStatus": "verified",
    "notes": "Variable font axes are added via animatorProps.addVariableFontAxis(tag) (AE 26.0+), never addProperty(\"ADBE Text VF Axis N\") -- AE throws \"Can not add a property\" for the latter. The returned property holds the axis's ABSOLUTE value (e.g. wght 100-900), not an offset. Fully keyframe each axis before adding the next -- addProperty/addVariableFontAxis invalidates prior sibling references in the same group. ALWAYS clamp keyframe values to the font's real fontObject.designAxesData [min,max] -- a naive target*0.5 guess (e.g. width 100 -> 50) can fall below the font's actual floor (e.g. wdth min 75), writing an invalid design-space coordinate; AE has crashed scrubbing/editing such a keyframe (verified empirically, 2026-07-21). Also skip adding a second 'Variable Font Axes' animator if one already exists -- stacking duplicates is never the intent."
  },
  {
    "id": "add-layer-marker",
