...
- [x] Select no layers. Ask a question. Confirm Claude doesn't hallucinate effect/keyframe data that isn't there.

### Error Feedback
- [x] Send a message that generates a script. Manually introduce an error at a specific line in `.session/ai-action.jsx`. Run it. Let it fail.
- [x] Click "Fix Last Error". Confirm the sent message contains the annotated script with `// <-- ERROR HERE` on the correct line.
- [x] Trigger a SyntaxError (use `const` in a script). Click "Fix Last Error". Confirm the ES3 hint appears in the prompt.
- [x] Delete `.session/ai-action.jsx`. Click "Fix Last Error". Confirm it falls back gracefully (error string only, no crash).
...
- [x] Ask `"add a camera with shallow depth of field"`. Confirm the script uses `ADBE Camera Options Group` / `ADBE Camera Depth of Field`.
...
- [ ] Add an in-panel way to switch providers after initial selection.
